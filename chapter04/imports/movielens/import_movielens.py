...
class MovieImporter(GraphDBBase):
    def __init__(self, argv):
        super().__init__(command=__file__, argv=argv)
        self._ia  = Cinemagoer() #IMDb(accessSystem='http', reraiseException=True) 
    ...
    def import_movies(self, file):
        ...
        with open(file, 'r+') as in_file:
            ...
            next(reader, None) # this avoids the columns name 
            #print(reader)
            with self._driver.session() as session: 
                self.execute_without_exception("CREATE CONSTRAINT FOR (a:Movie) REQUIRE a.movieId IS UNIQUE; ")
                self.execute_without_exception("CREATE CONSTRAINT FOR (a:Genre) REQUIRE a.genre IS UNIQUE; ")
                ...
                for row in reader:
                    try:
                        if row: 
                            ...
                            print(movie_id, imdb_id, type(movie_id), type(imdb_id))
                        ...
                        if i == 10:
                            tx.commit() 
                            print(j, "movie detaisl imported")
                            i = 0 
                            tx = session.begin_transaction() 
                    except Exception as e:
                        print(e, row, reader.line_num)
...
if __name__ == '__main__':
    importing = MovieImporter(argv=sys.argv[1:])
